...
function Page() {
    ...
    const handleSubmit = async () => {
        try {
            ...
            const response = await axios.post("http://localhost:3000/api/request-document", {
                date: date
            })
            if (response.data && response.data.document) {
                setDocument(response.data.document)
            } else {
                setError("No document found for this date")
            }
        } catch (err) {
                ...
                {document && (
                    <div className="p-6 bg-gray-50 rounded-lg border border-gray-200">
                        <div className="flex items-center mb-4">
                            ...
                            <h3 className="ml-2 text-lg font-medium text-gray-800">{document.name || "Document Available"}</h3>
                        </div>

                        <p className="text-sm text-gray-600 mb-6">{document.description || `Document for ${date} is ready for download.`}</p>
